#include "spi_driver.h"
#include "nrf_drv_gpiote.h"

static bool CS_trig = false;

static void spi_event_handler(nrf_drv_spi_evt_t const* p_event, void* p_context)
{
    spi_xfer_done = true;
}


void spi_init(void) {
    nrf_drv_spi_config_t spi_config = NRF_DRV_SPI_DEFAULT_CONFIG;
    spi_config.ss_pin           = NRF_DRV_SPI_PIN_NOT_USED;
    spi_config.miso_pin		= PIN_MISO;
    spi_config.mosi_pin		= PIN_MOSI;
    spi_config.sck_pin		= PIN_SCKL;
    spi_config.frequency	= NRF_DRV_SPI_FREQ_1M;	//Maximum for MAX86141 is 4MHz
    spi_config.mode		= NRF_DRV_SPI_MODE_0;	//Clock is idle low, and data is read in the MAX chip on the rising edge, and written on the falling edge. From my understanding it should be mode 1 but it seems like only mode 0 works.
    spi_config.bit_order	= NRF_DRV_SPI_BIT_ORDER_MSB_FIRST;
    spi_config.irq_priority     = APP_IRQ_PRIORITY_HIGH;

    APP_ERROR_CHECK(nrf_drv_spi_init(&spi, &spi_config, spi_event_handler, NULL));
    NRF_LOG_INFO("SPI initialized successfully");
    NRF_LOG_FLUSH();
}

/*
 * @brief Function to read one byte from a certain SPI slave at the specified register address.
 * @param cs_pin Which chip select pin to pull low
 * @param address 8-bit register address
 * @param rx_buf Address of where to save the data
 */
uint8_t spi_read_reg(uint8_t cs_pin, uint8_t address) {
	spi_xfer_done = false;
	
	//Bring cs_pin low
	nrf_gpio_pin_clear(cs_pin);

	//Send the register address and set the read bit
	uint8_t tx_buf[2] = {address, 0xFF};
	uint8_t m_rx_len = 3;
	uint8_t m_rx_buf[m_rx_len];

	//Start SPI transfer
	APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, tx_buf, 2, m_rx_buf, m_rx_len));

	//Wait until transfer is complete
	while (!spi_xfer_done) { __WFE(); }

	//Bring cs_pin high
	nrf_gpio_pin_set(cs_pin);

	return m_rx_buf[2];

}

/*
 * @brief Function to write one byte to a specific register address
 * @param cs_pin Which chip select pin to pull low
 * @param address 8-bit register address
 * @param data Byte to write at address
 */
void spi_write_reg(uint8_t cs_pin, uint8_t address, uint8_t data) {
	spi_xfer_done = false;

	//Bring cs_pin low
	nrf_gpio_pin_clear(cs_pin);

	//Set the register address, read bit is zero, and the data  byte
	uint8_t tx_buf[3] = {address, 0, data};

	//Start the SPI transfer
	APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, tx_buf, 3, NULL, 0));

	//Wait until transfer is complete
	while (!spi_xfer_done) { __WFE(); }

	//Bring cs_pin high
	nrf_gpio_pin_set(cs_pin);

}

/*
 * @brief Function to burst read several bytes from a certain SPI slave at the specified register address.
 * @param cs_pin Which chip select pin to pull low
 * @param address 8-bit register address
 * @param rx_buf Address of where to save the data
 * @param num_bytes How many byte to burst read
 */
void spi_burst_read(uint8_t cs_pin, uint8_t address, uint8_t* rx_buf, uint8_t num_bytes) {
	spi_xfer_done = false;
	
	//Bring cs_pin low
	nrf_gpio_pin_clear(cs_pin);

	//Send the register address and set the read bit
	uint8_t tx_buf[1] = {address};
	uint8_t m_rx_buf[num_bytes + 1];


	//Start SPI transfer
	APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, tx_buf, 1, m_rx_buf, num_bytes + 1));

	//Wait until transfer is complete
	while (!spi_xfer_done) { __WFE(); }

	//Bring cs_pin high
	nrf_gpio_pin_set(cs_pin);

	//Copy the recieved data (minus the first byte) to the output buffer (rx_buf)
	memcpy(rx_buf, &m_rx_buf[1], num_bytes);	
}

/*
 * @brief Function to start a SPI transfer. 
 * @param cs_pin Which GPIO pin of the nRF to pull low. This assumes that the GPIO pin is already configured as an output.
 * @param tx_buf Data to send to slave.
 * @param tx_len How many bytes to send.
 * @param rx_buf Where to save data from slave. If not needed, NULL is acceptable.
 * @param rx_len How many bytes to listen for. Note, the length includes the bytes during transmission.
 */
void spi_transfer(uint8_t cs_pin, uint8_t* tx_buf, uint8_t tx_len, uint8_t* rx_buf, uint8_t rx_len) {
	spi_xfer_done = false;
	
	//Bring cs_pin low
	nrf_gpio_pin_clear(cs_pin);

	//Start SPI transfer
        ret_code_t err_code = nrf_drv_spi_transfer(&spi, tx_buf, tx_len, rx_buf, rx_len);
	APP_ERROR_CHECK(err_code);
        if(err_code != NRF_SUCCESS)
            return;

	//Wait until transfer is complete
	while (!spi_xfer_done) { __WFE(); }

	//Bring cs_pin high
	nrf_gpio_pin_set(cs_pin);
}

/*
 * @brief Function to start a SPI transfer. 
 * @param tx_buf Data to send to slave.
 * @param tx_len How many bytes to send.
 * @param rx_buf Where to save data from slave. If not needed, NULL is acceptable.
 * @param rx_len How many bytes to listen for. Note, the length includes the bytes during transmission.
 */
void spi_transfer_no_cs(uint8_t* tx_buf, uint8_t tx_len, uint8_t* rx_buf, uint8_t rx_len) {
	spi_xfer_done = false;

	//Start SPI transfer
        ret_code_t err_code = nrf_drv_spi_transfer(&spi, tx_buf, tx_len, rx_buf, rx_len);
	APP_ERROR_CHECK(err_code);
        if(err_code != NRF_SUCCESS)
            return;

	//Wait until transfer is complete
	while (!spi_xfer_done) { __WFE(); }
}