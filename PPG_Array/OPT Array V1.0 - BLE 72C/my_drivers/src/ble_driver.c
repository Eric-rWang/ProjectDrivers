...
typedef struct {
    uint16_t len;
    uint8_t  data[NUS_PACKET_SIZE];
} tx_frame_t;

static tx_frame_t txq[TXQ_DEPTH];
static volatile uint8_t txq_head = 0, txq_tail = 0;

static inline bool txq_empty(void){ return txq_head == txq_tail; }
static inline bool txq_full(void){ return (uint8_t)(txq_head + 1) % TXQ_DEPTH == txq_tail; }

static void txq_push(const uint8_t *buf, uint16_t len){
    if (txq_full()){
        // Strategy: drop oldest to keep up (or count a stat).
        txq_tail = (uint8_t)(txq_tail + 1) % TXQ_DEPTH;
    }
    txq[txq_head].len = len;
    memcpy(txq[txq_head].data, buf, len);
    txq_head = (uint8_t)(txq_head + 1) % TXQ_DEPTH;
}

static void try_drain_tx(void){
    while (!txq_empty()){
        uint16_t n = txq[txq_tail].len;
        ret_code_t err = ble_nus_data_send(&m_nus, txq[txq_tail].data, &n, m_conn_handle);

        if (err == NRF_SUCCESS){
            txq_tail = (uint8_t)(txq_tail + 1) % TXQ_DEPTH;
            continue; // try to send the next one immediately
        }
        if (err == NRF_ERROR_RESOURCES){
            // TX queue is full; wait for BLE_GATTS_EVT_HVN_TX_COMPLETE
            break;
        }
        if (err == NRF_ERROR_INVALID_STATE || err == NRF_ERROR_NOT_FOUND){
            // Not connected or notifications not enabled yet; leave items queued.
            break;
        }
        if (err == NRF_ERROR_DATA_SIZE || err == NRF_ERROR_INVALID_PARAM){
            // MTU not negotiated yet (or too small). You can split here if desired.
            // For simplicity, stop draining until MTU update.
            break;
        }
        APP_ERROR_CHECK(err);
    }
...
void send_data_nus(uint8_t* data_array, uint16_t length) {
    ...
    // Guard against MTU not yet negotiated
    if (length > m_ble_nus_max_data_len) {
        NRF_LOG_WARNING("NUS frame %u > MTU payload %u — dropping",
...
void nus_send_buffer(void) {
        ...
        // RTT Writing: comment out for BLE APP
        if (DEBUG_RTT) {
