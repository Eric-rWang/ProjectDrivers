...
void max86141_init(void) {
	...
	max86141_setup_interrupts();
        ...
        uint8_t PIN_CS_PPG[2] = {SPI_CS_3, SPI_CS_4};
 ...
 //       }
	...
	//[3:2]		LED2_RGE
	//[1:0]		LED1_RGE
	spi_write_multi_reg(PIN_CS_PPG, MAX86141_LED_RANGE_1, (0x03 << 2) | (0x03), NUM_MAX_IC);
	...
	spi_write_multi_reg(PIN_CS_PPG, MAX86141_LED_SEQUENCE_REG_1, (0x01 << 4) | (0x02), NUM_MAX_IC);
...
void max86141_interrupt_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action) {
	...
	if (intStatus & 0x80) { //A FULL RDY
		...
		//Read PPG FIFO
                uint8_t PIN_CS_PPG[2] = {SPI_CS_3, SPI_CS_4};
                max86141_fifo_multi_parser(PIN_CS_PPG, 2);
