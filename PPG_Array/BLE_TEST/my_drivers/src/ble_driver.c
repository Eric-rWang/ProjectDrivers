...
void send_data_nus(uint8_t* data_array, uint16_t length) {
    ...
    /* Split the payload into notifications no larger than the negotiated MTU. Chunk sizes are
       forced even so a 16-bit sample is never split across two notifications. */
    while (offset < length)
    {
        uint16_t chunk = length - offset;
        if (chunk > m_ble_nus_max_data_len)
        {
            chunk = m_ble_nus_max_data_len & ~1u;
        }

        uint16_t to_send;
        do {
            to_send  = chunk;
            err_code = ble_nus_data_send(&m_nus, &data_array[offset], &to_send, m_conn_handle);

            /* No peer connected or notifications not enabled: drop the batch instead of blocking. */
...
void send_data_cus(tms_msg_tx_t message, uint8_t* extra_data, uint8_t length_extra_data) {
	//uint32_t err_code = ble_cus_data_send(&m_cus, (uint8_t) message);
	uint8_t data[1 + length_extra_data]; 
	data[0] = (uint8_t) message;
	memcpy(&data[1], extra_data, length_extra_data);

	uint16_t len = sizeof(data);
	uint32_t err_code = ble_cus_data_send(&m_cus, data, &len, m_conn_handle);
	NRF_LOG_INFO("Sent TMS");
}
...
void send_system_state(void) {
	...
	switch (state) {
		case SYSTEM_STATE_ALL_ON:
			send_data_cus(TMS_MSG_TX_NORMAL_STATE, NULL, 0);
			break;
		case SYSTEM_STATE_PERIPHERALS_OFF:
			send_data_cus(TMS_MSG_TX_SHUTDOWN, NULL, 0);
			break;
		default:
			break;
	}
